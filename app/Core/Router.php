...
class Router {
    ...
    public function dispatch() {
        ...
        error_log('Método HTTP final: ' . $method);
        
        error_log('Rotas disponíveis:');
        ...
        foreach ($this->routes as $route) {
            error_log("Testando rota: {$route['method']} {$route['path']}");
            if ($this->matchRoute($route['path'], $uri, $params) && $route['method'] === $method) {
                ...
                error_log("Chamando controller: " . $controller . ", action: " . $action);
        ...
        error_log('Nenhuma rota encontrada para: ' . $uri . ' [método: ' . $method . ']');
    ...
    private function getRequestMethod() {
        ...
        if ($method === 'POST' && isset($_POST['_method'])) {
            ...
            error_log("Método alterado para: " . $method . " via _method");
        }
